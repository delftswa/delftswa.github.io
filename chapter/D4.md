...
Judging from this we can state that Play has not that much been adopted as major other players.
Still working as a developer pays off, the salaries range in $90k-$180k a year.

This metric does not say everything and should only be taken as an indication.
It could be that there is a lot more adoption of Play, but this is not listed on job sites.

Another question while looking at this data was, what language should we program in if we want to become a Play Framework developer?
We looked at the 198 openings and then searched if they required Java or Scala as a skill.
