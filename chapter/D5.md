...
This section will address what does Play offer now for Reactive web applications, 
and we will provide a perspective on the evolution of Play in the future.

### Reactive
One of the exiting features of Play is it's Reactive foundation. 
Play is part of Typesafes Reactive Platform, as part of their view on the 
...
we need to build Responsive, Resilient, Elastic and Message Driven systems.

![Reactive Manifesto](http://www.reactivemanifesto.org/images/reactive-traits.svg)

With Play, Typesafe adresses these aspects and makes it 
...
Where Concurrency previously was hard to get right Play and Scala handle much of this for you.

#### Blocking IO and heavy computation
When creating a web application you often find that you need to do IO, or heavy computation.
Traditionally programmers would block on this operation, and continue only when the IO or computation completed.
...
that allows the main HTTP *execution context* to handle new requests, even when lots of IO is in progress.
The image below [by Julien Richard-Foy](https://www.packtpub.com/books/content/handling-long-running-requests-play) illustrates this: 
...
While basic IO requires only some minor usage of Scala's Futures (or Promises) which can be then hand of to asynchronous actions, 
...
Using execution contexts related work can be grouped, keeping the default context fast and responsive, 
while still waiting for legacy blocking operations on other threads, in a transparent way.

![Execution Contexts](reactive/2400OS_05_08.png)

Blocking operations should be prevented in general however. 
...
It is used to hangle incoming requests as streams of Bytes, which finally get converted to a Result, 
...
Using Iteratee's many different data pipelines can be made. Take for example Gzip filters: 
...
but lets say that it is ideal for multi actor applications (like chat) and it allows for blocking operations
to be executed on backbone machines, essentially converting blocking IO to non-blocking IO.
...
To show asses whether Play is ready to evolve we look at how evolution could affect Play, 
and which risks and costs might apply. We also give some directions of what is on the [roadmap](https://docs.google.com/document/d/11sVi1-REAIDFVHvwBrfRt1uXkBzROHQYgmcZNGJtDnA/pub) currently.
...
Obviously, when creacting a Product you need to consider your current users. 
...
#### Containing Change
Since then, Play 2.0 has evolved and backwards compatiblity or _upgradability_ has always
been a major concern. Play has stabilized significantly and has modularised internal components.
...
<!-- SoC and Enc are mentioned in Rozansky -->
containing future changes into their logical place. 

### Allow Variability
By practicing the strategy for variability, evolutionability increases as well.
General approaches to variability include _making elements replacable_, 
_parameterising operation with configuration_ and _breaking processes into steps_.

In Play a lot of effort has put into not to depend on specific libaries. 
...
as these libraries will be easier to upgrade if the is less strictly coupled integration.
...
Enumerator (produce data) → Enumeratee (map data) → Iteratee (consume data)
# This can be easily transformed into this:
...
- continuous integration (=> Plays own documation is updated every 10 minutes!)
<!-- see: https://www.playframework.com/documentation/2.3.x/Translations#Deploying-documentation-to-playframework.com -->

### Risks
A risk for evolution of Play could be that any of its depedencies stops to be developed.
Luckily Play depends on only a few major libaries like Apache Commons,
