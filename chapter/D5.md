...
that allows the main HTTP *execution context* to handle new requests, even when lots of I/O is begin executed.
...
It is used to handle incoming requests as streams of bytes, which finally get converted to a result, 
...
but (very briefly summarised) that is ideal for multi-actor applications (like chat) and it allows for blocking operations
...
#### Upgradability
To maintain the user base with the framework as it develops, it is important for the users to be able to install newer versions of the framework without too much trouble.
