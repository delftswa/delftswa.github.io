...
###Abstract
This chapter explains and describes various views and perspectives of the Play framework that together aim at providing a broad architectural overview of the framework. 
The stakeholder and context view show that there are few but very prominent stakeholders in the project and that the framework itself does not have many dependencies. 
It is shown that Play is in a transition to a more modular design that improves scalability and speed, while trying to maintaining backwards compatibility. 
In the concurrency view is explained that Play aims at evolving to a reactive and entirely stateless web applications framework and that this is partly achieved by moving to an evented execution model of HTTP requests. 
Finally an overview of Play's past and future is provided from which can be concluded that these transitions are in line of proposed development roadmaps.
...
but separate application modules that can even be deployed separately, leverage horizontal scalability. 
Also, unused modules can be disabled, lowering resource usage. 
...
The overlaying text is dynamic and shows the commits in the currently hovered changes.
...
Also existing modules disappeared, as was the case with Twirl and Anorm. 
The main trend is adding modules instead of extending existing modules, and sometimes modules are split into smaller submodules. 
Altogether the developers have done a great job to keep modules small and focussed.

### Who's helping?
As any developer knows, the best tool for learning is looking up your questions on [Stackoverflow](http://www.stackoverflow.com).
...
Both seem reasonable: Play is very well documented, and Spring MVC has wide adoption due to Spring, convincing people to use that whole platform.
...
After the build all Unit and Integration tests are run and the result is reflected on Github, marking the pull request as ready to merge, or indicates it if the build or tests are failing.
...
### Context View
The context view that Play resides in is mainly dependent on the projects that are developed and maintained (SBT, Netty) by Typesafe itself. 
...
Communication about the framework is done using two external organisations, [Google Groups](https://groups.google.com/forum/#!forum/play-framework) mailing list forum and [GitHub](https://github.com/playframework/playframework). The latter mainly serves for development, however, issues and bugs are reported here.
...
But Play is indeed very capable in handling Reactive data streams and Futures, also known as Promises.
...
Traditionally programmers would block on this operation, and continue only when the I/O or computation completed.
...
that allows the main HTTP *execution context* to handle new requests, even when lots of I/O is in progress.
...
For example, when we defer a heavy computation, we need to put it onto a different execution context, 
...
Using Iteratees, many different data pipelines can be made. Take for example Gzip filters: 
...
These streams can be long running, but the tasks themselves are small, and ideal for an evented execution model.
...
which really allows for Message Driven applications. [Akka](http://akka.io/) is a subject on its own, 
...
but is stored in the Cookie, at the client side. Authenticity is ensured by signing the Cookie with
a secret key only available on the server. 
...
Another way state is reduced is actually by the Iteratees library as an Iterator encapsulates and protects its internal state: Iteratees are Monads.
...
to keep up which the fast changing requirement to modern Web application.

#### Containing Change
Since then, Play 2.0 has evolved and backwards compatibility or _upgradability_ has always
been a major concern. Play has stabilised significantly and has modularised internal components.
As discussed in the [Features](#features) section many features are now modules - 
essentially practicing _separation of concerns_ and _encapsulation_ - 
containing future changes into their logical place. 

<!-- SoC and Enc are explicitly mentioned in Rozansky -->

### Allow Variability
By practicing the strategy for variability, the evolvability increases as well.
...
In Play a lot of effort has put into not to depend on specific libraries. 
...
as these libraries will be easier to upgrade if there is less strictly coupled integration.

Not only is Play itself extremely configurable at both run compile time as run-time,
...
but more importantly allows for `include` statements. This eases evolution as independent
...
For users of the framework, Play's Iteratees library is a very good tool for this. 
...
then transcode the file on a heavy back-end machine and finally notify all connected clients of the new video.
The communication could be handled by Akka. 
If later another stabilisation phase has to be inserted, this can easily be done.

### Extensibility
Next to variability for internal components is having extension points for external functionality.
...
External developers can create extensions, or thin wrappers to other libraries so that the main developers can focus on the core functionalities.
...
Tools like, automated builds and testing, software configuration management and continuous integration.
...
For reliable change software configuration management is essential, to reproduce the development environment later on.
...
## Conclusion
Play is a framework that is constantly evolving to ensure that their users can integrate the latest and state of the art software technologies and methodologies into their Play web applications. 
Part of this evolution is transforming to a modular design and reducing the module size, to ensure scalability, reducing resource usage and increasing (compilation) speeds. 
...
Allowing variability, upgradability and maintaining backwards compatibility are crucial for Play to provide user with the latest technologies and maintain its arguably small but sophisticated and exiting user base.
