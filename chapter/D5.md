...
To support reliable change, good tools need to be provided by the framework according to Rozanski.
From a developer viewpoint we described a number of these tools in section [how to contribute](#how-to-contribute?).
In this section we described the tools used for automated build and automated testing.

The software configuration is managed in a own language, which can be used in Ivy or Maven style.
Under the hood, the software can use both repositories to download configurations.
For using the software products, a [repository](https://repo.typesafe.com/typesafe/releases/) can be used from Typesafe, the repository can be accessed in maven2 style.
...
The only part that is continuously integrated is the documentation, which is updated every ten minutes.

- software configuration management (Ivy(/Maven))
- automated build process
- automated testing
- continuous integration (=> Plays own documentation is updated every 10 minutes!)
