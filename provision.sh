...
apt-get install git curl unzip nodejs -y >> /tmp/provision.log 2>&1
...
rvm install ruby-2.1.5 >> /tmp/provision.log 2>&1
rvm use ruby-2.1.5 --default >> /tmp/provision.log 2>&1
...
echo "fincally run 'jekyll serve --force_polling'"
